...
class TrackCoverRequest
{
    ...
    private function record(Request $request, Response $response, int $responseTime): void
    {
        try {
            CoverRequest::create([
                'server_id'     => config('server-monitor.server_id'),
                'url'           => $request->fullUrl(),
    ...
    private function shouldSkip(Request $request): bool
    {
        ...
        foreach (config('server-monitor.exclude_paths', []) as $pattern) {
    ...
    {
        return in_array($status, config('server-monitor.exclude_status_codes', []), true);
    }
}
